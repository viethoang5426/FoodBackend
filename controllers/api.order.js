...
const create_payment_url = async (req, res, next) => {
  ...
  try {
    if (
      ...
      return res
        .status(400)
        .json({ status: 0, msg: "iduser hoặc PaymentMethodID không hợp lệ" });
    }

    const finUser = await mdUser.userModel.findById(userID);
    const finPaymentMethod = await mdPaymentMethod.paymentMethodModel.findById(
      PaymentMethodID
    );
    ...
    if (finPaymentMethod.MethodName != "VNPAY") {
      return res
        .status(400)
        .json({ status: 0, msg: "truyền đúng paymentmethod id có name VNPAY" });
    }

    if (!cartID || !Array.isArray(cartID) || !finUser) {
      return res
        .status(400)
        .json({ status: 0, msg: "Dữ liệu giỏ hàng hoặc user không hợp lệ" });
    }
    ...
    for (const itemCart of cartID) {
      ...
      const findProduct = await mdProduct.productModel.findById(
        fincart.ProductID
      );
      if (
        findProduct.StockQuantity <= 0 &&
        itemCart.Quantity > findProduct.StockQuantity
      ) {
        return res
          .status(400)
          .json({ message: "số lượng sản phẩm đã hết hoặc không đủ" });
      }
    ...
    return res
      .status(404)
      .json({ status: 0, error: "Đã xảy ra lỗi " + error.message });
  }
...
const create_payment_cod = async (req, res, next) => {
  ...
  try {
    ...
    if (finPaymentMethod.MethodName != "COD") {
      return res
        .status(400)
        .json({ status: 0, msg: "truyền đúng paymentmethod id có name COD" });
    }
  ...
  try {
    for (const iterator of cartID) {
      ...
      if (quantityCart > quantityProduct) {
        return res
          ...
          .json({
            message:
              "Không thể thực hiện ,do số lượng sản phẩm nào đó không đủ hoặc đã hết",
          });
      } else {
      }
    }
    ...
    let data_cart = [];
    for (const iterator of cartID) {
      const finCart = await mdCart.cartModel
        .findById(iterator)
      ...
      const cartObject = {
        ...
        ProductID: {
          ...
          Ratting: finCart.ProductID.Ratting,
        },
    ...
    const newORDERDETAIL = new mdOrderDetail.orderDetailModel(
      newOrderDetailData
    );
    ...
    for (const iterator of cartID) {
      ...
      try {
        ...
        return res
          .status(404)
          .json({ error: "Đã xảy ra lỗi 2" + error.message });
      }
    }
...
const vnpay_return = async (req, res, next) => {
  ...
  if (secureHash === signed) {
    try {
      ...
      for (const iterator of cartID) {
        ...
        try {
          ...
          return res
            .status(404)
            .json({ error: "Đã xảy ra lỗi " + error.message });
        }
      }
    } catch (error) {
      return res
        .status(404)
        .json({ error: "Đã xảy ra lỗi 1 " + error.message });
    }
    ...
    res.redirect("http://192.168.1.7:3000/successOrder?mess=ok");
  } else {
...
const getByIdUser = async (req, res, next) => {
  ...
  try {
    ...
    const order = await mdOrder.orderModel
      .find({ UserID: userID })
...
const putOrderStatus = async (req, res, next) => {
  ...
  try {
    const order = await mdOrder.orderModel.findByIdAndUpdate(
      orderId,
      { Status },
      { new: true }
    );
